...
orders_status = OC('SubType=Список_переменных','MainEbilockObject','Coordinates','Next','Prev',)

controllers = OC('SubType=Контроллер','MainEbilockObject','OC_Type','varlist',)

mks = OC('SubType=Концентратор\n','MainEbilockObject','Boss','МКС-МУ_1','МКС-МУ_2',
         'Coordinates','disconnect_table',)
table_losses_ycu = OC('SubType=Таблица_потерь_связи_с_концентраторами','MainEbilockObject',
                        'graph',)
oc_plates = OC('SubType=Плата ','MainEbilockObject','SubType',)
unknown_ycu = OC('Неизвестный_концентратор\n','MainEbilockObject','table')
...
@pytest.mark.parametrize('var', orders_status.VarsObjs, ids=orders_status.get_ids())
...
@pytest.mark.parametrize('var', controllers.VarsObjs, ids=controllers.get_ids())
...
@pytest.mark.parametrize('var', mks.VarsObjs, ids=mks.get_ids())
...
@pytest.mark.parametrize('var', oc_plates.VarsObjs, ids=oc_plates.get_ids())
...
@pytest.mark.parametrize('var', unknown_ycu.VarsObjs, ids=unknown_ycu.get_ids())
